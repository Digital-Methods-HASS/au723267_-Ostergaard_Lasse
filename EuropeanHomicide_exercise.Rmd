...
#The data looks rather tidy. Only Italy has a code
```
...
Western_Europe is the data that i will be using.

Im using ggplot to make a visualisation of the data. I will use the geom_smooth for geometry, since it gives a nice and readable visualisation.

I will use labs to give my visualisation a title that suits the data. 

```{r plot-homicides, eval = TRUE}

#Making a smooth plot and adding a title to the plot 

Western_Europe %>% 
ggplot(aes(x=Year,y=homicides_per_100k))+
geom_smooth(aes(fill=Entity))+
labs(title="Homicide rate in Western Europe,1300-2016")
...
```{r Western_Europe_facet_wrap, eval = TRUE}
#Using the facet_wrap function to split the countries in to seperate charts 
...
```{r Midyear_and_danish_monarchs, eval = TRUE}
# YOUR CODE HERE:
read_csv("data/kongerækken_ark-kopi.csv")
kongerækken <- read_csv("data/kongerækken_ark-kopi.csv")

#Creating the midyear column
kongerækken_plotting <- kongerækken %>% 
mutate(duration_of_rule=end_of_reign-start_of_reign) %>% glimpse()


#Creating the midyear column
kongerækken_plotting <- kongerækken_plotting %>% 
mutate(midyear=end_of_reign-(duration_of_rule)/2)


glimpse(kongerækken_plotting)

#Creating visualisation

kongerækken_plotting %>%
  ggplot(aes(x=midyear,y=duration_of_rule))+geom_smooth()+
  labs(title="Danish monarchs duration of rule, 958-2023",x="Middle year of reign",y="Duration of rule")

##It looks like the monarchs are ruling for a longer time, while the homicide rate has decreased in Europe
...
Based on the visualisations that i have produced, it would seem that that we are more civilized today. The homicide rate has decreased in all of the western european countries we have looked at in this dataset. It also seems that the duration of rule for the danish monarchs have increased in the same period. There might not be a correlation between the two things. The only correlation might be that the homicide rate goes down and the duration of rule goes up, then if only looking at the visualisations you might make the connections that the rate of murders on monarchs has decreased. Even that assumption might not be a 100% plausible. To sum up, based on the visualisations it seems that we are more civilized today.  
